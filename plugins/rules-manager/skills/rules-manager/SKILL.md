...
Claude Code の `.claude/rules/` ディレクトリにルールファイルを作成・管理し、`settings.json` を編集するスキル。

## 基本原則

1. **必ずスコープを確認する** — 作業を始める前に、ユーザーに以下を聞く：
   - **プロジェクトレベル**（`.claude/rules/`）: チームで共有、git にコミットされる
   - **ユーザーレベル**（`~/.claude/rules/`）: 個人用、すべてのプロジェクトに適用
   確認なしにファイルを作成してはいけない。
...
| ユーザー | `~/.claude/rules/` | 自分だけ（全プロジェクト共通） |
